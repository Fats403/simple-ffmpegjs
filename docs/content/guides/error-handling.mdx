...
| `FFmpegError` | FFmpeg command fails (or times out via `timeoutMs`) | `stderr`, `command`, `exitCode`, `details` |
| `MediaNotFoundError` | File not found or unreadable | `path` |
| `ExportCancelledError` | Export, snapshot, or keyframe extraction aborted via `AbortController` | — |
| `TranscodeError` | `transcode()` or an [audio operation](/api/audio-operations) fails | `code` (`INVALID_PATH` \| `INPUT_MISSING` \| `FFMPEG_NOT_FOUND` \| `NO_VIDEO_STREAM` \| `NO_AUDIO_STREAM` \| `ANALYSIS_FAILED` \| `TIMEOUT` \| `NONZERO_EXIT` \| `SIGNAL` \| `ABORTED`), `stderr`, `exitCode`, `details` |
