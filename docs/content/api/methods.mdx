...
| `quality` | `number` | - | JPEG quality 1-31, lower is better (`.jpg`/`.jpeg` only; ffmpeg's default when omitted) |
| `timeoutMs` | `number` | `300000` | Hard timeout, SIGKILL-backed |
| `signal` | `AbortSignal` | - | Cancel; rejects with `ExportCancelledError` |

Supported formats: `.jpg`, `.jpeg`, `.png`, `.webp`, `.bmp`, `.tiff`. Fails with a clear error if `time` is beyond the end of the video (instead of silently returning a path to nothing).
...
Both modes accept `timeoutMs` (default 5 minutes, SIGKILL-backed) and `signal`
...
| `timeoutMs` | `number` | - | Hard timeout per ffmpeg run (SIGKILL-backed) |
| `logLevel` | `string` | - | ffmpeg `-loglevel`; progress callbacks keep working |
...
Preparing a preview writes the same temp files a real export would (text files,
ASS subtitles, unrotated video intermediates) and cleans them up afterwards.
The project is left ready to export: `preview()` followed by `export()`, or
multiple `export()` calls from one `load()`, all work.
