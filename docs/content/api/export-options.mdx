...
| `videoCodec` | `string` | `'libx264'` | Video codec (`libx264`, `libx265`, `libvpx-vp9`, `prores_ks`, hardware codecs). Validated up front: a typo fails with the list of valid values instead of an ffmpeg stderr dump |
...
| `audioOnly` | `boolean` | `false` | Export audio only (no video stream). When `audioCodec` is not set, the codec is chosen from the output extension (`.mp3` → `libmp3lame`, `.wav` → `pcm_s16le`, `.flac`, `.ogg`, `.opus`, `.m4a`/`.mp4` → `aac`) |
...
| `timeoutMs` | `number` | — | Hard timeout per ffmpeg run, SIGKILL-backed. No timeout when omitted (renders can legitimately take a long time) |
| `logLevel` | `string` | — | ffmpeg `-loglevel` (`quiet`…`debug`). Progress callbacks keep working at quiet levels (`-stats` is added automatically) |
