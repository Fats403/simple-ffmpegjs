...
```ts
{
  ...
  transition?:
    | { type: string; duration?: number }  // duration defaults to 0.5s
    | string;               // shorthand: "fade" ≡ { type: "fade", duration: 0.5 }
}
```

All [FFmpeg xfade transitions](https://trac.ffmpeg.org/wiki/Xfade) are supported, and the
`type` is validated up front — a typo'd name fails with the list of valid values instead
of a raw xfade error mid-render.
...
```ts
{
  ...
  transition?:              // Crossfade INTO this clip, same as video clips
