...
//   sampleRate: 48000,          // Hz
//   channels: 2,
//   attachedPic: false,         // true when the only "video" is embedded cover art (MP3 album art)
...
Requires a playable video stream: audio-only input, or an MP3 whose only "video" stream is embedded cover art, rejects with `code: "NO_VIDEO_STREAM"` — use `web-audio` for those.

### `web-audio` preset

The audio-side equivalent for uploads that are podcasts, voiceovers, or music. Produces AAC in an MP4-family container — `outputPath` must end in `.m4a` or `.mp4`:

```ts
await SIMPLEFFMPEG.transcode("./upload.mp3", {
  outputPath: "./normalized.m4a",
  preset: "web-audio",
});
```

- First audio stream only (`-vn -map 0:a:0`), cover art dropped
- AAC at 128 kbps (override with `audioBitrate`), source channel count preserved — a mono voiceover stays mono
- `-movflags +faststart`
- Video-only options (`crf`, `videoBitrate`, `scale`) are rejected; input without audio rejects with `code: "NO_AUDIO_STREAM"`
...
| `preset` | `"web-mp4" \| "web-audio"` | — | Mutually exclusive with `customArgs`. |
...
| `NO_VIDEO_STREAM` | `web-mp4` input has no playable video (audio-only, or cover-art-only "video") |
| `NO_AUDIO_STREAM` | `web-audio` input has no audio stream |
